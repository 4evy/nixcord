...
const React = {
  createElement(type: unknown, props: Record<string, unknown> | null, ...children: unknown[]) {
    ...
    return typeof type === 'function' ? type(merged) : merged;
  },
...
const runtime = {
  importValue(moduleName: string, importedName: string, localName: string): unknown {
    ...
    if (/react/i.test(moduleName)) return importedName === 'default' ? React : component(localName);
...
try {
  ...
  const javascript = transpiler.transformSync(payload.code);
  const execute = new Function('__runtime', 'React', javascript);
