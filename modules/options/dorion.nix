...
in
{
  options.programs.nixcord.dorion = {
    ...
    configDir = lib.options.mkOption {
      type = lib.types.path;
      description = "Directory containing Dorion's `config.json`.";
    };
    ...
    blur = lib.options.mkOption {
      ...
      default = "none";
      description = "Window blur effect: none, blur, or acrylic.";
    };
    ...
    pushToTalkKeys = lib.options.mkOption {
      type = lib.types.listOf lib.types.str;
      default = [ "RControl" ];
      description = "Keys that activate push-to-talk when `pushToTalk` is enabled.";
      example = [
        "RControl"
        "F1"
      ];
    ...
    profile = lib.options.mkOption {
      type = lib.types.str;
      default = "default";
      description = "Dorion profile to open.";
    };
    ...
    extraSettings = lib.options.mkOption {
      type = lib.types.attrsOf jsonFormat.type;
      default = { };
      description = "Extra entries for Dorion's `config.json`, using its native setting names. These override values generated by the other Dorion options.";
    };
  };
}
